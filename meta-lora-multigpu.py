        ...
        for step, train_batch in enumerate(train_dataloader):
            ...
            if (args.meta == 'False' or (step == 0)):
                ...
                if ((step - 1) % args.validation_interval == 0):
                    ...
                    print(
                        f"Epoch: {epoch}, Step: {step}, Rank: {torch.distributed.get_rank()}, eval Loss: {eval_loss}"
                    )
                ...
                print(
                        f"Epoch: {epoch}, Step: {step}, Rank: {torch.distributed.get_rank()}, loss_of_training_samples = {train_gathered_loss}"
                    )
                ...
                with torch.no_grad():
                    ...
                    print(
                        f"Epoch: {epoch}, Step: {step}, Rank: {torch.distributed.get_rank()}, Original ex_weight: {ex_weight}"
                    )
                    if (min(ex_weight) < 0.0): # one weight is below 0
                        ex_weight = torch.sub(ex_weight, 2 * min(ex_weight))
                        print(
                        f"Epoch: {epoch}, Step: {step}, Rank: {torch.distributed.get_rank()}, Retified ex_weight: {ex_weight}"
                    )
                    else:
                    ...
                    print( # the weight of each sample
                        f"Epoch: {epoch}, Step: {step}, Rank: {torch.distributed.get_rank()}, Final ex_weight: {ex_weight}"
                    )
                ...
                print(
                        f"Epoch: {epoch}, Step: {step}, Rank: {torch.distributed.get_rank()}, Final loss: {final_loss}"
                    )
